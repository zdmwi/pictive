<template>
    <div class="flex flex-col rounded-lg bg-white h-auto w-2/4 overflow-hidden shadow-lg mb-4 cursor-pointer" @click="isOpen=!isOpen;">
        ...
        <div v-for="comment in userCmmt.slice(0,3)" :key="comment.id" class="bg-gray-100 rounded-lg overflow-hidden shadow mx-4 mb-2 pb-2">
            <div class="flex justify-start px-6 py-4">
                <div class="inline-block bg-gray-300 h-8 w-8 rounded-full cursor-pointer"></div>
                <span class="inline-block text-gray-900 text-sm leading-none pl-4 pt-2">Joana Willis</span>
        ...
        <button class="h-10 w-40 my-2 text-sm bg-transparent hover:bg-gray-500 text-gray-700 font-semibold hover:text-white border border-gray-500 hover:border-transparent rounded">
        See More
        </button>
        </div>
    </div>
